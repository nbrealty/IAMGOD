...
# Narrative & worldbuilding — see `docs/STORY_BIBLE.md`

The full **Story Bible** (novela storytelling framework N1–N11 + supernatural entity bible E1–E12)
lives in [`docs/STORY_BIBLE.md`](docs/STORY_BIBLE.md), to keep this always-loaded file lean. Read it
before any narrative/story task. **It is context only — do NOT implement code, systems, missions,
dialogue, assets, DB, world-clocks, religious adaptation, or cultural visual design from it until
Nelson explicitly asks.** Status tags there: **[CANON]** / **[DIR]** / **[PROV]** / **[RESEARCH]**.

Canon anchors (the guardrails, so day-to-day work doesn't trip them — details + provisional/
research flags are in the bible):
- **Genre:** Brazilian-novela structure; strong reveals **reclassify** an established identity
  ("true but incomplete"), they don't erase it. Misdirection is allowed but must stay **fair**.
- **Create-a-Character = God reincarnated** into a human (limited memory, human POV, no guaranteed
  moral wisdom). Default origin **The Second Heartbeat** (ordinary soul → baby dies/arrests at birth
  → original soul peacefully leaves → God takes the body → heartbeat returns → citywide beacon "I am
  awake"). Entities sense it differently.
- **Cosmology (top-down, 5 levels):** cosmic/trans-state (God, Lucifer, loyal & fallen angels) ·
  territorial sovereigns (**Molache** in LA) · cultural/ancestral powers (kept culturally DISTINCT) ·
  urban collectives (**The Audience**) · human conduits/institutions. Entities show through human
  systems, and the board advances on its own via per-entity **world clocks**.
- **"Demon" is specific:** only angels who fell with Lucifer. Molache, orixás, Exus, Pombagiras,
  Vodou lwa, ancestral/Indigenous sacred beings, ghosts, urban collectives are **NOT** demons even
  when hostile (characters may misclassify; lore keeps the distinction).
- **Key players:** Lucifer = trans-state tester/opponent who wants **vindication** from God, not
  God dead (real warmth + real manipulation; begins as a helpful guide). Molache = sovereign of
  sacrifice who asks "how can I **use** God?"; undone by a **freely given gift** (no debt/audience/
  exchange). Broken Choir = angelic search network. Yara is **human**, not an entity.
- **Cultural rule (hard):** never merge Tongva/Acjachemen/Tataviam/Chumash/Candomblé/Umbanda/
  Quimbanda/Vodou/Hoodoo; never invent a secret "true mythology" for a living tradition; Puvungna is
  a **living sacred place**, not a dungeon; culturally-rooted beings are research/consultation-gated.
